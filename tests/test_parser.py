...
def test_declaration():
    assert ps('int x = 42').expect(syn_vdecl()) == Declaration(
        Variable(const=False, type=DataType(TypeToken.INT), name='x'),
        IntLiteral(42, Anywhere()), Anywhere()
    )

    assert ps("const byte x = 'B'").expect(syn_vdecl()) == Declaration(
        Variable(const=True, type=DataType(TypeToken.BYTE), name='x'),
        IntLiteral(ord('B'), Anywhere()), Anywhere()
    )

    assert ps('bool x[5]').expect(syn_vdecl()) == Declaration(
        Variable(const=False, type=ArrayType(TypeToken.BOOL), name='x'),
        ArrayInitializer(ArrayType(TypeToken.BOOL), IntLiteral(5, Anywhere())),
        Anywhere()
    )

    with raises(ParserError): ps('int x').expect(syn_vdecl())
    with raises(ParserError): ps('const x = 5').expect(syn_vdecl())
    with raises(ParserError): ps('aint x = 5').expect(syn_vdecl())
